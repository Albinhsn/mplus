...
int main() {
  ...
  ModelData model = {};
  bool result = sta_parse_wavefront_object_from_file(
      &model, "./data/unarmed_man/unarmed.obj");
  if (!result) {
  ...
  TargaImage image = {};
  result = sta_read_targa_from_file(
      &image, "./data/unarmed_man/Peasant_Man_diffuse.tga");
  if (!result) {
  ...
  sta_glBufferData(GL_ARRAY_BUFFER, sizeof(VertexData) * model.vertex_count,
                   model.vertices, GL_STATIC_DRAW);

  sta_glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
  sta_glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(u32) * model.vertex_count,
                   model.indices, GL_STATIC_DRAW);

  sta_glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float),
                            (void *)0);
  sta_glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(float),
                            (void *)(3 * sizeof(float)));
  sta_glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float),
                            (void *)(5 * sizeof(float)));
  ...
  sta_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, image.width, image.height, 0,
